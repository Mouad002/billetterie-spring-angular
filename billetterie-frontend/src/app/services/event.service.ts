...
export class EventService {
  ...
  public getEventDetails(id : number):Observable<AppEvent>{
    return this.http.get<AppEvent>(environment.backendHost+"/organizer/events/"+id)
  }
  public getEventImageUrl(imageName: string | undefined): string {
    if (!imageName) return '';
    return `${environment.backendHost}/readImages/Events/${imageName}`;
  }
  public getEventImage(imageName: string | undefined): Observable<Blob> {
    if (!imageName) {
      return throwError(() => new Error('Image name is undefined'));
    }
    return this.http.get(`${environment.backendHost}/readImages/Events/${imageName}`, {
      responseType: 'blob'
    });
  }
}
